    ...
    <section className="relative pt-40 pb-24 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          ...
          >
            Strategically architecting data-driven experiences that drive growth and engagement for global sports organizations.
          </motion.p>
          <motion.div variants={itemVariants} className="flex flex-wrap gap-6">
            <Link
              to="/case-studies"
              className="bg-brand text-white px-8 py-4 rounded-full text-lg font-bold hover:scale-105 active:scale-95 transition-all flex items-center gap-3 group"
            >
              View Case Studies
            ...
            <Link
              to="/story"
              className="bg-surface-soft border border-surface-border text-white px-8 py-4 rounded-full text-lg font-bold hover:bg-surface-border transition-all"
            >
              The Story
            </Link>
          </motion.div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {IMPACT_STATS.map((stat, idx) => (
            <motion.div
              ...
              transition={{ duration: 0.8, delay: 0.5 + (idx * 0.1), ease: [0.22, 1, 0.36, 1] }}
